...
use App\Pancarta;
use App\CarteleraPancarta;
use App\Cartelera;

class CartelerasController extends \TCG\Voyager\Http\Controllers\VoyagerBaseController
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        $cartelera_pancartas = CarteleraPancarta::where("cartelera_id",$id)->pancartas()->get();//Cartelera::find($id)->pancartas()->orderBy("id")->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        //$c = Cartelera::find($id)->pancartas()->get();
       // dd( count( explode(",",$request->input("pancartas")) ) );

        CarteleraPancarta::where("cartelera_id",$id)->delete();
        if( $request->input("pancartas") != null ){
            $pancartas = explode(",",$request->input("pancartas"));
            for( $i = 0 ; $i < count( $pancartas )  ; $i++ ){
                $cartelera = new CarteleraPancarta;
                $cartelera->cartelera_id = $id;
                $cartelera->pancarta_id = $pancartas[$i];
                $cartelera->save();
            }
        }
